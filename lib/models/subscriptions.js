...
module.exports.list = (listId, start, limit, queryData, callback) => {
    listId = Number(listId) || 0;
    if (!listId) {
        return callback(new Error('Missing List ID'));
    }

    tableHelpers.list('subscription__' + listId, ['*'], 'email', queryData, start, limit, (err, rows, total) => {
        if (!err) {
...
module.exports.update = (listId, cid, updates, allowEmail, onlyExisting, callback) => {
    ...
    fields.list(listId, (err, fieldList) => {
        ...
        fields.getValues(fields.getRow(fieldList, updates, true, true, onlyExisting), true).forEach(field => {
            keys.push(field.key);
            values.push(field.value);
        });
